...
- Iterates over all node types via `NodeType::loadMultiple()`.
- Skips types where the `canvas_override.enabled` third-party setting is
  `FALSE`.
- Returns an array keyed by permission machine name.
...
[
  'use canvas override for article' => [
    ...
    'description' => t('Edit per-node Canvas layouts on %type nodes.', ['%type' => 'Article']),
  ],
]
```
...
Extends Canvas's `ComponentTreeLoader` to support per-node layouts.
