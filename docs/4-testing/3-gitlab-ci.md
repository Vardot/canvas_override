...
| test | `varbase-e2e-test-drupal-core` | Drupal Standard profile + Canvas, runs `cucumber.js` |
| test | `varbase-e2e-test-drupal-cms` | Drupal CMS distribution + Canvas, runs `cucumber.drupalcms.js` |
...
- The DrupalCI `composer` job reads `.gitlab-ci/patches.json` via
  `_COMPOSER_PATCHES_FILE`, so every PHPUnit and `varbase-e2e-test-drupal-core` job inherits
  the patched `drupal/canvas`.
- The `varbase-e2e-test-drupal-cms` job builds a fresh `drupal/cms` project and
  registers the same patch through `composer config extra.patches` before
  requiring Canvas.

### How the browser jobs seed the site

The varbase-e2e jobs build the site and apply the two test recipes with
...
gitlab-ci-local --file .gitlab-ci-local.yml varbase-e2e-dry-run
